...
export default class Interpreter extends NodeVisitor {
  ...
  public visitProcedureCall(node: ProcedureCall) {
    ...
    const ar = new ActivationRecord(procName, ARType.PROCEDURE, procSymbol.scopeLevel + 1)

    const formalParams = procSymbol.formalParams
    const actualParams = node.actualParams

    formalParams.forEach((paramSymbol, index) => {
      const argumentNode = actualParams[index]
