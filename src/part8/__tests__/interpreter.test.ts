...
describe('interpreter', () => {
  ...
  test('test_expression8', () => {
    const interpreter = makeInterpreter('- 3')
    const result = interpreter.interpret()
    expect(result).toBe(-3)
  })
  test('test_expression9', () => {
    const interpreter = makeInterpreter('+ 3')
    const result = interpreter.interpret()
    expect(result).toBe(3)
  })
  test('test_expression10', () => {
    const interpreter = makeInterpreter('5 - - - + - 3')
    const result = interpreter.interpret()
    expect(result).toBe(8)
  })
  test('test_expression11', () => {
    const interpreter = makeInterpreter('5 - - - + - (3 + 4) - +2')
    const result = interpreter.interpret()
    expect(result).toBe(10)
  })
  test('test_no_expression', () => {
  ...
  test('test_expression_invalid_syntax2', () => {
    expect(() => {
      const interpreter = makeInterpreter('1 (1 + 2)')
      interpreter.interpret()
    }).toThrow()
  })
})
